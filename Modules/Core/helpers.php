...
use Modules\Apps\Entities\MobileVersion;
...
if (! function_exists('getCurrentMokbileVersion')) {

    function getCurrentMokbileVersion(){
        
        if(request()->header("version") && request()->header("system")){

            $system = request()->header("system");
            $production_status = setting("mobile_apps_updates","$system.production_status") && setting("mobile_apps_updates","$system.production_status") == "on" ? true : false;

            if(setting("mobile_apps_updates","$system.version") == request()->header("version") && !$production_status){
                return true;
            }
        }
